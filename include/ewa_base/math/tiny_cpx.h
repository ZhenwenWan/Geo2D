...
public:
	...
	tiny_cpx<T> conj() const { return tiny_cpx<T>((*this)[0], -(*this)[1]); }
...
struct cpx_promote_real<H, 1> : public H
{
	...
	typedef tiny_cpx<typename H::type> promoted;
	typedef scalar type;
};
...
template<typename T>
struct is_cpx { static const bool value = false; };

template<typename T>
struct is_cpx < tiny_cpx<T> > { static const bool value = false; };

template<typename T>
struct remove_cpx { typedef T type; };

template<typename T>
struct remove_cpx < tiny_cpx<T> > { typedef T type; };
...
{
	template<typename T> 
	inline T abs(const ew::tiny_cpx<T>& o)
	{
		return std::abs((const complex<T>&)(o));;
	}

	template<typename T> 
	inline ew::tiny_cpx<T> sqrt(const ew::tiny_cpx<T>& o)
	{
		return std::sqrt((const complex<T>&)o);;
	}
