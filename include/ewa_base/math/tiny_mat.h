...
struct mat_promote_real<H, 1, R,C> : public H
{
	...
	typedef tiny_mat<typename H::type, R,C> promoted;
	typedef scalar type;
};
...
template<typename T,int R,int C>
tiny_mat<T,C,R> transpose(const tiny_mat<T,R,C>& v)
{
	tiny_mat<T,C,R> res;
...
template<typename X,typename Y>
typename vec_promote_n<X,Y,3>::promoted operator*(const tiny_mat<X,4,4>& lhs,const tiny_vec<Y,2>& rhs)
{
	typename vec_promote_n<X,Y,3>::promoted res;
	res[0]=lhs(0,0)*rhs[0]+lhs(0,1)*rhs[1]+lhs(0,3);
	res[1]=lhs(1,0)*rhs[0]+lhs(1,1)*rhs[1]+lhs(1,3);
	res[2]=lhs(2,0)*rhs[0]+lhs(2,1)*rhs[1]+lhs(2,3);
	return res;
}
...
public:
	...
	{
		basetype& m3(*this);
		m3(0, 2) += m3(0, 0)*v[0] + m3(0, 1)*v[1];
		m3(1, 2) += m3(1, 0)*v[0] + m3(1, 1)*v[1];
	}
	...
	const vec3& GetDir(unsigned n) const
	{
		EW_ASSERT(n < 3);
		basetype& m3(*this);
		return *(vec3*)&m3(0, n);
	...
	// 构造矩阵(eu, ev, en)
	// eu表示局部坐标u的单位矢量在全局坐标系的表示，ev/en类似
	// (eu, ev, en) * (u, v, n)^T = eu * u + ev * v + en * n
	// 即将矢量的局部坐标表示转换为全剧坐标表示
	void LoadUVN(const vec3& eu, const vec3& ev, const vec3& en)
	{
		GetDir(0) = eu;
		GetDir(1) = ev;
		GetDir(2) = en;
...
public:
	...
	static const type_mat<X, 4, 4> id;
	...
	void LoadAxis(int d)
	{
		...
		tiny_mat<X,4,4> res;
		res((d+1)%3,0) = 1.0;
		res((d+2)%3,1) = d<3?-1.0:1.0;
		res((d+3)%3,2) = d<3?-1.0:1.0;
		res(3, 3) = 1.0;
		(*this) = res;
	}

	void LoadDirection(const vec3& dir, const vec3& cnt = vec3(0, 0, 0))
	{
		...
		vec3 n = dir / dir.length();
		vec3 u = ew::cross(z, n); u = u / u.length();
		vec3 v = ew::cross(n, u); v = v / v.length();
	...
	void TransformWithUVN(const vec3& eu, const vec3& ev, const vec3& en)
	{
		type_mat<X, 4, 4> res;
		res.GetDir(0) = eu;
		res.GetDir(1) = ev;
	...
	template<typename Y>
	void Translate(Y x, Y y, Y z)
	{
		basetype& m4(*this);
		m4(0,3)+=m4(0,0)*x+m4(0,1)*y+m4(0,2)*z;
		m4(1,3)+=m4(1,0)*x+m4(1,1)*y+m4(1,2)*z;
		m4(2,3)+=m4(2,0)*x+m4(2,1)*y+m4(2,2)*z;
	}
	...
	template<typename Y>
	void Rotate(double deg,const tiny_vec<Y,3>& dir_)
	{
		if(deg==0.0) return;
		double L=dir_.length();
		if(L==0.0) return;
		...
		tiny_vec<double,3> dir=dir_/L;

		double k0 = dir[0] * dir[0];
		double k1 = dir[1] * dir[1];
		double k2 = dir[2] * dir[2];
	...
	vec3 GetUnitDir(unsigned n) const
	{
		EW_ASSERT(n < 3);
		const basetype& m4(*this);
		vec3 dir(m4(0,n),m4(1,n),m4(2,n));
		dir /= dir.length();
		return dir;
	}
	...
	template<typename Y>
	tiny_vec<Y, 3> Transform33(const tiny_vec<Y, 3>& v) const
	{
		const basetype& m(*this);
		return tiny_vec<Y, 3>(
	...
	static type_mat<X,4,4> make_scale(double s)
	{
		type_mat<X, 4, 4> m4;
		m4.Scale(s);
		return m4;
	...
	static type_mat<X, 4, 4> make_scale(double s1, double s2, double s3)
	{
		type_mat<X, 4, 4> m4;
		m4.Scale(s1,s2,s3);
		return m4;
	}
	static type_mat make_translate(double s1,double s2,double s3)
	{
		type_mat<X, 4, 4> m4;
		m4.Translate(s1,s2,s3);
		return m4;
	}

	template<typename Y>
	static type_mat<X, 4, 4> make_rotate(double deg, const tiny_vec<Y,3>& dir)
	{
		type_mat<X, 4, 4> m4;
		m4.Rotate(deg, dir);
		return m4;
...
template<typename X>
const type_mat<X, 4, 4> type_mat<X, 4, 4>::id;
...
namespace tl
{
	...
	template<typename T,int R,int C> struct is_pod<ew::type_mat<T,R,C> > : public value_type<true>{};
};
