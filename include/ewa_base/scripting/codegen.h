...
class DLLIMPEXP_EWA_BASE CG_GGVar;
...
public:
	...
	CG_Variable* add_export(const String& name_,const String& module_);

	CG_Variable* add_local(const String& name_);
	String add_local_by_prefix(const String& pre);
...
public:
	...
	int add(const String& v)
	{
		aLLVar.push_back(Variant(v));
		return aLLVar.size() - 1;
	}
...
public:
	...
	void add_sbase(const String& s,int d)
	{
		CG_Variable* q = new CG_Variable(VAR_SBASE, s);
		q->index = d;
		add(q);
	}
...
class VisExtraHandler : public ObjectData
{
public:

	virtual void on_undefined_var(TNodeVisitorCG& vis, const String& var);
	virtual void on_set_global(TNodeVisitorCG& vis, const String& var);
	virtual void on_get_global(TNodeVisitorCG& vis, const String& var);


};

class VisExtraHandlerVariable : public VisExtraHandler
{
public:
	...
	bst_set<String> a_undefined_vars;
	bst_map<String,int> a_var_global;
};


class DLLIMPEXP_EWA_BASE CodeGen : public CGenState
{
...
public:
	...
	bst_map<String,int> cg_exports;

	CodeGen();
	CodeGen(const CGenState& cgs);
	...
	bool parse(const char* p1,const char* p2);

	bool parse(TNode_val_function* q);
	...
	void add_b(int level);
	void add_c(int level);
	...
	bool can_fastlink(const String& s);
	...
	bool is_command_var(TNode_var* pvar);
...
public:
	CG_Local& local;
	CG_Block& scope;
	LockScope2(CG_Local& c, CG_Block& b) :local(c), scope(b)
	{
		local.enter2(scope);
	}
	~LockScope2()
...
public:
	CG_Local& local;
	CG_Block& scope;
	LockScope1(CG_Local& c, CG_Block& b) :local(c), scope(b)
	{
		local.enter1(scope);
	}
	~LockScope1()
