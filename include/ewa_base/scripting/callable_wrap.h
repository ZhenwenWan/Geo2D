...
public:
	...
	CallableWrap(int t):CallableData(t){}

	int __getarray(Executor& ks, int);
...
public:
	...
	CallableDataBaseT()
		:CallableWrap(type_flag<type>::value)
	{

	}

	CallableDataBaseT(const T& v)
		:CallableWrap(type_flag<type>::value)
		,value(v)
	{
...
#define DEFINE_CALLABLE_WRAP(T)\
template<>\
class DLLIMPEXP_EWA_BASE CallableWrapT<T> : public CallableDataBaseT<T>\
{\
public:\
	typedef T type;\
...
#define DEFINE_CALLABLE_WRAP_T(V)\
template<typename T>\
class DLLIMPEXP_EWA_BASE CallableWrapT<V<T> > : public CallableDataBaseT<V<T> >\
{\
public:\
	typedef V<T> type;\
...
template<>
class DLLIMPEXP_EWA_BASE CallableWrapT<Logger>;

DEFINE_CALLABLE_WRAP(ColonData)
