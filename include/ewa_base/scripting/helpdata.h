#ifndef __H_EW_SCRIPTING_HELPDATA__
#define __H_EW_SCRIPTING_HELPDATA__

#include "ewa_base/scripting/callable_data.h"

EW_ENTER


class DLLIMPEXP_EWA_BASE CallableData_helpdata : public CallableData
{
public:

	CallableData_helpdata();
	CallableData_helpdata(const String& s, const String& v);

	indexer_map<String, String> helpmap;

	void parse(const String& name,const String& desc);

	virtual bool ToValue(String& s, int n = 0) const;

	virtual void Serialize(SerializerHelper sh);

private:
	void _init();


	DECLARE_OBJECT_INFO(CallableData_helpdata, ObjectInfo);
};

EW_LEAVE
#endif
