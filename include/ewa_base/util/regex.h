...
public:
	...
	class DLLIMPEXP_EWA_BASE item_tree
	{
	public:

		item_tree(){}
		item_tree(const char* p) :str(p, p){}

		Match::item str;
		intrusive_list<item_tree> link;
		LitePtrT<item_tree> parent;
		LitePtrT<item_tree> next;
	};
	...
	const char* orig_c_str() const { return orig_str.c_str(); }

	item_tree* get_result(){ return &result; }
	const item_tree* get_result() const { return &result; }
	bool get_matched_item(String& d, const String& w) const;
