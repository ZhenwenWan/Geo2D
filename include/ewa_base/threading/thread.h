...
public:
	...
	static CpuAffinity set_affinity(const CpuAffinity& n);
	...
	static void set_cwd(const String& folder);
	static String get_cwd();
	static String& ref_cwd();
...
public:
	...
	typedef Functor<void()> functor_type;

	typedef arr_1t<functor_type> InvokerGroup;

	// Start a group of threads.
	bool activate(InvokerGroup& g);

	// Start 1 thread calling functor().
	bool activate(const functor_type& fac,size_t n=1);
...
class DLLIMPEXP_EWA_BASE ExternalThreadLocker
{
public:
	ExternalThreadLocker();
	~ExternalThreadLocker();
...
DEFINE_OBJECT_NAME(Thread, "thread");
