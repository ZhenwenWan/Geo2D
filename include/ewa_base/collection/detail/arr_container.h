...
public:
	...
	template<typename It>
	inline void assign(It first_,It last_)
	{
		assign(first_,std::distance(first_,last_));
	}

	inline iterator insert(const_iterator where_,const T& val_)
	{
	...
	inline iterator erase(iterator first_)
	{
		return erase(first_,1);
	}

	template<typename It>
	inline iterator erase(It first_, It last_)
	{
	...
	inline iterator begin(){ return iterator(m_base); }
	inline iterator end(){ return iterator(m_end1); }
	inline reverse_iterator rbegin(){ return reverse_iterator(m_end1 - 1); }
	inline reverse_iterator rend(){ return reverse_iterator(m_base - 1); }
	...
	inline pointer data(){ return m_base; }

	inline T& operator[](size_type n)
	{
		EW_ASSERT_LT(n, size_type(m_end1 - m_base));
		return m_base[n];
	}

	inline void return_initialized_buffer(size_type n)
	{
		EW_ASSERT_LE(n, size_type(m_end2 - m_end1));
		m_end1+=n;
	}

	inline T* get_uninitialized_buffer(size_type n);
...
protected:
	...
	inline T* _alloc_copy(size_t capacity_, T* first_, T* last_)
	{
		return _alloc_copy(capacity_,first_,last_-first_);
	}

	inline void _dealloc_destroy(T* p1, T* p2, T* p3)
	{
		if(p1)
		{
	...
	inline void _check_not_empty()
	{
		if(m_base==m_end1)
		{
			Exception::XError("arr_container_is_empty",false);
		}
	}
	...
	{
		return detail::SizeHelper<A>::gen2(n0, n1, this->get_allocator().max_size());
	}
...
template<typename T,typename A,bool pod>
inline T* arr_container<T, A, pod>::_alloc_copy(size_t capacity_, const T& val_, size_t count_)
{
	if(capacity_==0) return NULL;

	capacity_ = detail::SizeHelper<A>::adj(capacity_);
...
template<typename T,typename A,bool pod>
template<typename IT>
inline T* arr_container<T, A, pod>::_alloc_copy(size_t capacity_, IT first_, size_t count_)
{
	if(capacity_==0) return NULL;
...
template<typename T,typename A,bool pod>
inline void arr_container<T, A, pod>::shrink_to_fit()
{
...
template<typename T,typename A,bool pod>
inline T* arr_container<T,A,pod>::get_uninitialized_buffer(size_type n)
{
	size_t n0 = size();
	size_t n2 = capacity();
...
inline void arr_container<T,A,pod>::reserve(size_type capacity_)
{
	...
	size_t n0=size();
	size_t n1=detail::SizeHelper<A>::adj(capacity_);
...
template<typename T,typename A,bool pod>
inline void arr_container<T, A, pod>::_resize_gen(size_type count_, const T& val_)
{
	size_t n0=size();
	size_t n1 = detail::SizeHelper<A>::adj(count_);

	T* p1=_alloc_copy(n1,m_base,m_end1);
	T* p2=p1+n0;
	T* p3=p1+count_;

	xmem<T>::uninitialized_fill(p2,p3,val_);

	_clear_and_rawset(p1,p3,p1+n1);
}
...
template<typename T,typename A,bool pod>
inline void arr_container<T, A, pod>::_append_gen(const T& val_)
{
	size_t n0=size();
...
template<typename T,typename A,bool pod>
template<typename It>
inline typename arr_container<T, A, pod>::iterator arr_container<T, A, pod>::_append_gen(It first_, size_type count_)
{
	size_t n0=size();
