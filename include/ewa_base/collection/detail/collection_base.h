...
namespace mem
{
	...
	It2 copy_n(It1 src, size_t n, It2 dst)
	{
		return std::copy_n(src, n, dst);
	}

	template<typename It1, typename It2>
	It2 uninitialized_copy_n(It1 src, size_t n, It2 dst)
	{
		return std::uninitialized_copy_n(src,n,dst);
	}
...
public:
	...
	static inline size_t gen(size_t sz)
	{
		...
		if (sz <= 32)
		{
			return adj(sz - 2 + (sz >> 1),2);
		}
		if(sz<(1<<16))
		{
			return adj(sz - 4 + (sz >> 1),4);
		}
		return adj(sz + (sz >> 1),1<<12);
	}
...
public:
	...
	template<int N>
	struct ind2sub_helper
	{
		size_t dmsz[N];

		ind2sub_helper(const arr_xt_dims& dims)
		{
			dmsz[0] = dims.dim[0];
			for (int i = 1; i < N; ++i)
			{
		...
		bool check(size_t ind) const
		{
			return ind < dmsz[N - 1];
		}

		template<typename I>
		void ind2sub(I& sub, size_t ind) const
		{
			sub[0] = ind % dmsz[0];
			for (int i = 1; i < N; ++i)
			{
	...
	template<int N>
	ind2sub_helper<N> get_ind2sub_helper() const
	{
		return ind2sub_helper<N>(*this);
	}

	template<int N, typename I>
	void ind2sub(I& sub, size_t ind) const
	{
		ind2sub_helper<N> helper = get_ind2sub_helper<N>();
		return helper.template ind2sub<I>(sub, ind);
	}
	...
	{
		return is_matrix() && (dim[0] == dim[1]);
	}

	const size_t* ptr() const {return dim;}

	bool operator== (const arr_xt_dims& _d) const
	{
...
class DLLIMPEXP_EWA_BASE container_base : public mp_obj
{
public:
...
public:
	...
	static const bool allocator_is_empty = V;
...
public:
	typedef A allocator_type;
	static const bool allocator_is_empty = false;

	alloc_base() {}
	alloc_base(const alloc_base& o){}
...
public:
	...
	bool operator==(const rac_iterator& rhs) const
	{
...
{
	return rac_iterator<T,D,C>(IteratorDirection<T, D>::add((T*)it1.get(), n));
}

template<typename T,bool C,bool D>
rac_iterator<T,D,C> operator-(rac_iterator<T,D,C> it1,intptr_t n)
{
	return rac_iterator<T,D,C>(IteratorDirection<T, D>::sub((T*)it1.get(), n));
}
...
public:
	...
	{
		return mem::uninitialized_copy_n(first_,count_,dest_);
	}
	...
	{
		return mem::copy_n(first_,count_,dest_);
	}
...
{
	template<typename B>
	void swap(ew::containerB<B>& v1, ew::containerB<B>& v2)
	{
		v1.swap(v2);
	}
...
#ifdef EW_WINDOWS
	template<typename T, bool D, bool C>
	struct pointer_traits< ew::rac_iterator<T, D, C> > : pointer_traits<T*>
	{
