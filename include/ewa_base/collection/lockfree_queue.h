...
public:
	...
	void resize(size_t n)
	{
		if(n>0)
		{
			...
			if ((sz - sizeof(SRSW_queue_impl)) / sizeof(T) != n - 1)
			{
				Exception::XBadAlloc();
			}

			void* p=mp_alloc_throw(sz);
			reset(p,sz,mp_free);
		}
		else
		{
...
void LockFreeQueue<T,P>::reset(size_type v)
{
	...
	lf_queue_header* p1=(lf_queue_header*)mp_alloc_throw(sizeof(lf_queue_header));
	...
	{
		mp_free(p1);
		Exception::XBadAlloc(sizeof(T)*v);
	}
