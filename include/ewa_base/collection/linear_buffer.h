...
public:
	...
	using basetype::wr_pos;
	using basetype::rd_pos;
	using basetype::sz_buf;
...
void skip_comment1(LinearBuffer<T>& lbuf) // comment //
{
	...
	if(lbuf.get()!='/')
	{
		Exception::XError(_hT("'/'_expected"));
		return;
	}
...
void read_integer(LinearBuffer<T>& lbuf,int64_t& vv,int& gg)
{
	...
	for(;;)
	{
		...
		if(d>10)
		{
			if(val=='.')
			{
				if(doted==1)
				{
					Exception::XError(_hT("invalid_number") + _hT(":") + _hT("two_dots"));
				}
	...
	if(kk==0)
	{
		Exception::XError(_hT("invalid_number"));
	}
...
void read_element(LinearBuffer<T>& lbuf,E& v)
{
	...
	if(val=='d'||val=='D'||val=='e'||val=='E')
	{
		...
		if(s2!=0)
		{
			Exception::XError(_hT("invalid_number") + _hT(":") + _hT("exponent_with_dot"));
		}
		s1=p2-s1;
	}
	else
...
void read_element(LinearBuffer<T>& lbuf,std::complex<E>& v)
{
	...
	if(val=='(')
	{
		...
		{
			lbuf.unget();
			Exception::XError(_hT("unexpected_character"));
		}
		skip_space(lbuf);
	...
	else
	{
		...
		{
			lbuf.unget();
			Exception::XError(_hT("invalid_complex"));
		}
	}
