...
public:
	...
	static const int MAX_DIM = arr_xt_dims::MAX_DIM;
};
...
public:
	...
	void set_dim(int d,int lo,int hi,int o)
	{
		...
		{
			Exception::XError(_hT("invalid_dim"));
		}
		dims.lo[d]=lo;
		dims.hi[d]=hi;
		dims.sz[d]=data.ds[o];
	...
	tmp_xt(arr_xt<T>& a, const tiny_vec<int32_t, 3>& shf)
	{
		reset(a, shf);
	}
	...
	void reset(arr_xt<T>& a, const tiny_vec<int32_t, 3>& shf)
	{
		data.pt=a.data();
		for(size_t i=0;i<6;i++)
		{
			...
			if (i < 3)
			{
				set_dim(i,shf[i],shf[i]+a.size(i),i);
			}
			else
			{
				set_dim(i, 0, a.size(i), i);
