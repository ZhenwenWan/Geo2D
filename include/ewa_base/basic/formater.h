...
class DLLIMPEXP_EWA_BASE StringBuilder :
	public MemoryBufferT<char,BufferPolicyChained<char,2048> >,
	public FormatHelper<StringBuilder,FormatPolicy2>
{
public:
	typedef MemoryBufferT<char, BufferPolicyChained<char, 2048> > basetype;
	...
	void append(const char* p1,const char* p2) { send(p1,p2-p1);}
	void append(const char* p) { send(p, ::strlen(p)); }
...
public:
	...
	template<typename T>
	FormatStateT(const T& v){FormatHandler<T>::init(*this,v);}
	...
	{
		FormatHandler<G>::handle_t(sb, o);
	}

	void str_append_t(double o);
	void str_append_t(float o);
	...
	void str_format_t(const G& o)
	{
		...
		if(f2[-1]=='n')
		{
			FormatHandler<G>::handle_n(sb.size(), o);
			return;
		}

		size_t n=size_t(n_fmt_width[0])+size_t(n_fmt_width[1]);
		S::Policy::format(sb,n,*this,o);
...
{
public:
	static const int value0=(N=='c'||N=='d'||N=='i'||N=='u'||N=='o'||N=='x'||N=='X')?1:0;
	static const int value1=(N=='f'||N=='F'||N=='e'||N=='E'||N=='g'||N=='G'||N=='a'||N=='A')?2:0;
	static const int value2=(N=='s')?3:0;
	static const int value3=(N=='p')?4:0;
	static const int value4=(N=='n')?5:0;
	static const int value5=(N=='m')?6:0;
	static const int value=value0|value1|value2|value3|value4|value5;
	...
	)
	{
		for(;handle(st);st.fmt_leave())
		{
			if(st.n_fmt_flag!=0)
			{
				...
				if ((st.n_fmt_type=lookup_table<lkt_format_characters>::test(st.p2[-1])) == 0)
				{
					st.fmt_error();
					continue;
				}
