...
inline uint32_t hash_impl_adjust(uint32_t v)
{
    return v;
}

inline uint32_t hash_impl_adjust(uint64_t v)
{
    return v^(v>>32);
}
...
public:
	...
	template<typename T>
	void handle_pod(const T& val)
	{
		update(&val, sizeof(T));
	}

	template<typename T>
	void hash(const T& val)
	{
		hash_t<T> h;
		handle_pod(h(val));		
	}
