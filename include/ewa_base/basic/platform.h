...
public:
	...
	static inline int32_t write(type v, const char* buf, size_t len)
	{
		DWORD nWrite(0);
		if (::WriteFile(v, buf, len, &nWrite, NULL) == FALSE)
		{
			return -1;
		}
		return nWrite;
	}
	static inline int32_t read(type v, char* buf, size_t len)
	{
		DWORD nRead(0);
		if (::ReadFile(v, buf, len, &nRead, NULL) == FALSE)
		{
			return -1;
		}
		return nRead;
...
public:
	...
	static inline int32_t write(type v, const char* p, size_t n)
	{
		return ::write(v, p, n);
	}
	static inline int32_t read(type v, char* p, size_t n)
	{
		return ::read(v, p, n);
	}
};
...
enum
{
	...
	FLAG_FILE_MKDIR		=FLAG_FILE_BASE<<7,
	...
	FLAG_FILE_WCA		=FLAG_FILE_WC|FLAG_FILE_APPEND,
	FLAG_FILE_WCT		=FLAG_FILE_WC|FLAG_FILE_TRUNCATE,
