...
public:
	...
	{
		typedef AtomicHelper<sizeof(T)> helper;
		return helper::compare_exchange(
			reinterpret_cast<typename helper::vpointer>(p),
			reinterpret_cast<typename helper::type&>(expected),
			v);
	}
};
...
template<typename T>
class AtomicIntT  : public mp_obj
{
public:
...
public:
	...
	{
		return (type)impl.exchange((intptr_t)v);
	}

	EW_FORCEINLINE  bool compare_exchange(type& expected,type v)
	{
		return impl.compare_exchange(*(intptr_t*)&expected,(intptr_t)v);
	}
	...
	{
		impl.store((intptr_t)v);
	}

protected:
	AtomicIntT<intptr_t> impl;
};
...
public:
	...
	EW_FORCEINLINE uint32_t count() const { return num; }
...
class DLLIMPEXP_EWA_BASE RefCounterWithMutex : public RefCounter
{
public:
	AtomicMutex mutex;
};
